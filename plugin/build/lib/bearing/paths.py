...
def plugin_root() -> str:
    ...
    stays correct across the two install shapes:

    - checkout / marketplace copy: `plugin.json` at the plugin tree root,
      Python package at `src/bearing/`
    - pip / pipx / uv-tool: `plugin.json` and `skills/` bundled *inside* the
      installed `bearing` package (see `plugin/setup.py`)

    A Cursor or Claude Code plugin cache is a third tree and is not consulted.
    The GUI plugin install feeds the agent runtime; it does not satisfy this
    walk.
    """
