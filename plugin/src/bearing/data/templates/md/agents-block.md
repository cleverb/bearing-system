...
- **Do not write to `{{decisions_path}}/` on the basis of inference.** Every promotion out of `{{shadow_dir}}/` requires human judgment that determines scope, present validity, lifecycle state, and EOCR function. One-click *execution* of that judgment (for example `bearing dispose` or MCP elicitation) is allowed; auto-promoting from confidence alone is not.
