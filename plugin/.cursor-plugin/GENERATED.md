...
Cursor reads a plugin manifest from `.cursor-plugin/plugin.json`. This format is required (rather than the bare Agent Plugins `plugin.json`) because BEARING ships subagents and hooks, which the portable v1.0.0 format does not cover.
