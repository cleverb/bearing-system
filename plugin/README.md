...
`plugin.json` is the only hand-maintained manifest. Everything in `.cursor-plugin/`, `.claude-plugin/`, `.codex-plugin/`, `agents/`, and `.codex/agents/` is generated by `bearing package` and carries a do-not-edit header. Regenerate rather than reconcile:
