...
Chosen option: **2**. This repository adopts numbered ADRs under the `docs/decisions/` tree, following the EOCR knowledge grammar (Entry, Operations, Contracts, Rationale) and the Decision Graph mechanics described in `/BEARING.md`. Records may live at the root or in descriptive category subdirectories; categories do not create separate ADR namespaces.
...
* Legacy decisions may be recovered retrospectively via the `decision-recovery` and `decision-interview` Skills, installed as part of the BEARING plugin.
* Both `NNNN-title.md` and `ADR-NNNN-title.md` filename forms are recognized recursively. IDs remain unique and sequential across the full corpus.

## Validation

* [x] `docs/decisions/index.json` reflects this ADR — regenerated by `bearing index`.
* [x] `AGENTS.md` points agents to this directory as the authoritative decision source.

## Implementation History

* **2026-08-18:** Added recursive category-directory support while retaining one repository-wide ADR namespace.
