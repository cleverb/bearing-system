...
This distinction matters because it's the direct answer to the objection every real adoption of this architecture eventually runs into: *we have fifteen years of code and almost none of it has ADRs — are you saying we have to document everything before this becomes useful?* No. The retrospective path recovers decision ancestry progressively, alongside normal work, without requiring the backlog to be cleared before the system starts paying for itself — and without ever letting what a machine inferred stand in for what the organization has actually declared authoritative. The `decision-recovery` Skill is the concrete implementation of this path; see its spec for the mechanics of extraction, evidence scoring, and the human review boundary that keeps the two graphs separate.
